...
When comparing the sessions for the same subject we have found samples where the level of athletic activity \(\texttt{Level}\) changes and the reported dominant leg (\texttt{DominantLeg}) changes over time, and cases where both \texttt{SpecInjury} and \texttt{InjDefn} change. We belive this cases are plausible and consider them to be true and part of the specificities of the dataset. There is nothing in the the source paper that makes us belive that these cases need to be considered a data quality issue. We leave them untouched.

On the other hand, we have found one case where the age gap between sessions is 10 years, which is longer than the duration of the study. We decided to check the whole dataset for inconsistencies between the age difference between sessions and the time of the sessions. Two samples had a significant difference (more than 1 year difference): Subject 100234 was 40 on a initial session and 43 in a session 1 year later. 200375 was a female of age 27 on one session and a 50 year old male subject on the next one, it is clearly two different people.

For the first case, we decided to override the the age of the second session and set it to 41. For the second case, we decided to create a new synthetic subject ID (300375) and assign the second session to the new subject. The number of sessions remains the same, but there is a new subject.

In addition we have observed that \texttt{SpecInjury} is empty, but \texttt{SpecInjury2} is populated on many sessions. For those cases, we assume that it is an input error and move the value from \texttt{SpecInjury} to \texttt{SpecInjury2}, that is, if an injury was reported in \texttt{SpecInjury}, we will take it as a primary injury, if not and \texttt{SpecInjury2} is populated, we will take that one. We will only assume a subject has no primary injury in a session if both \texttt{SpecInjury} and \texttt{SpecInjury2} are empty.

We inspected all of the columns in the metadata file to find outliers. Table \ref{tab:met-outlier-range} porvides a summary of the columns with outliers and the range used to detect outliers.

\begin{table}[h!]
    \centering
    \caption[Outlier detection range for tabular data]{Summary of outlier detection range for key variables.\label{tab:met-outlier-range}
    \begin{tabular}{lp{0.6\textwidth}}
        \hline
        ...
        Height & 120.0 & 196.5 \\
        Weight & 42.5 & 176.0 \\
        InjDuration (days) & 0.0 & 176.0 \\
        YearsRunning & 0.0 & 54.0 \\
        \hline
    \end{tabular}
\end{table}

Values outside of these ranges were considered unrealistic and were replaced with None. For example, ages of 255 years, heights of 999 cm, weights of 1564 kg, injury durations of 30,000 days (82 years), and years running of 999 were all identified as outliers and removed. These extreme values were detected visually using box-plots and by checking for plausibility against normal values for human adults.
