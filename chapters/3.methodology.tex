...
Additionally to the time-series data, the script also generates the set of 33 descriptive variables. These variables are the same as the ones in the JSON files in the source dataset.  This variable set was used to validate the results of the execution of \texttt{processing\_source\_data.m} by comparing the output with the source dataset. All variables are equal up to a relative tolerance of $1 \times 10^{-9}$ except for 35 \texttt{speed\_r} values, that are equal to $9 \times 10^{-2}$, this is negligible for human speed measured in m/s. This result supports the idea that the execution was successfully. We decide to discard the computed set of descriptive variables in favour of the set provided in the original dataset because the original set has 1832 samples. The data is ready for the exploratory data analysis stage.
...
- DominantLeg distribution skewed to Right leg as expected. We have few ambidextrous.
...
TODO: Do we need a visualization to support?
...
We override the age of the former to 41 and we create a new synthetic subject ID (300375) and assign the second session to the new subject.
...
Lastly, we introduce a surrogate key "id" that can be used across datasets to identify each sample. One sample is a session. The surrogate key is the concatenation of subject id with the filename in uppercase i.e. \"100001\_20110531T161051\". This will be needed when working with both time-series and metadata.
