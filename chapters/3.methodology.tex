...
% Logistic Regression
Logistic Regression models the log-odds of the target as a linear combination of the inputs. A sigmoid converts the score into a probability. This gives a simple, interpretable baseline that reacts to scaled feature magnitudes. The 40 mRMR-selected features were standardised, and class imbalance was addressed using class weights. The main hyperparameters were: inverse regularisation strength \texttt{C} ($10^{-3}$ to $10^{3}$), penalty (\texttt{L1} or \texttt{L2}), and solver (\texttt{liblinear} or \texttt{saga}). The solver-penalty pair must be compatible (e.g., \texttt{liblinear} supports L1/L2; \texttt{saga} is suitable for larger feature spaces).
...
\begin{itemize}
    ...
    % \item Multi modal <-- Decide if to remove
\end{itemize}
