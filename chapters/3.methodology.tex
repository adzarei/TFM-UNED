...
Multiple algorithms exist for DTW estimation, each with very different results. Through visual inspection we decided to choose XXXXXX as it was the most stable.

Unfortunately, the iterative nature of the algorithm resulted in very poor performance. The execution of the algorithm for the complete set of curves took more than 30 days of execution with the available compute resources even after parallelizing across 8 cores, which makes this option unfit or the resources requirements of the project. Sadly the mean curve was calculated as a fast alternative. This took. With more time and resources, the algorithm could be transformed into a matrix problem with better optimization possibilities.
