...
\texttt{processing\_source\_data.m} was run. Out of the 2506 sessions (Some subjects have more than one session), 2487 were processed successfully and 1813 had running data. Sessions without valid running data, or those where MATLAB preprocessing failed, were excluded. 

The output contains time-series dataset of the markers, joint angles and joint angular velocities, and the detected events of the gait cycle and on what frame do they occour. The output was processed using \texttt{pandas} and \texttt{kinetics toolkit} packages.
Figure \ref{fig:data-ext-marker-velo-angle-ts} shows an example of the raw marker data for 1 marker in the left shank cluster, and the joint angle and joint angular velocity for the left knee for one sample session. Figure \ref{fig:data-ext-l-knee-angle-events} shows fame 500 to 1000 for the left knee angle marked with the cycle events.
...
\begin{figure}[ht]
    \begin{centering}
    \includegraphics[width=0.5\columnwidth]{images/ex_l_knee_angle_events.png.png}
    \par\end{centering}
    \caption{Frames 500 to 1000 of the 3 axis: [0] X, [1] Y and [2] Z of the left knee angle annotated with touchdown (L_TD) and toe-off (L_TO) events.}
