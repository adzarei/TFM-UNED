...
% TODO: If there is value, we can explain how:
% - Descriptive variables are derived in matlab code
% - angles, angular velocities are derived in matlab code.
...
The outputs consist of time-series data for markers, joint angles, joint angular velocities, and detected gait cycle events with their corresponding frame indices. The outputs were further processed using the \texttt{pandas} and \texttt{kinetics-toolkit} Python packages to combine the event data with the timeseries data.

Figure \ref{fig:data-ext-marker-velo-angle-ts} illustrates an example from a single session, showing the raw marker data for one marker in the left shank cluster alongside the corresponding joint angle and angular velocity for the left knee. Figure \ref{fig:data-ext-l-knee-angle-events} shows frames 500-1000 of the left knee angle, annotated with the touchdown and toe-off events. Figure \ref{fig:data-ext-marker-3d} shows the 3D representation of 1 frame of the all of the marker data.

Additionally to the timeseries data, the script also generates the set of 33 descriptive variables. These variables are the same as the ones in the JSON files in the source dataset.  This variable set was used to validate the results of the execution of \texttt{processing\_source\_data.m} by comparing the output with the source dataset. All variables are equal up to a relative tolerance of $1 \times 10^{-9}$ except for 35 \texttt{speed_r} values, that are equal to $9 \times 10^{-2}$, this is neglectible for human speed measured in m/s. This result supports the idea that the exeuction was successfully. We decide to discard the computed set of descriptive variables in favour of the set provided in the original dataset because the original set has 1832 samples. The data is ready for the exploratory data analysis stage.
...
\begin{figure}[ht]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{3D representation of a frame with all marker data. Connecting lines have been added between markers. The left foot has been painted in blue and the right foot in red. From top to bottom: Pelvis, Left/Right Thigh, Left/Right Shank and Left/Right Foot.}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{1st left shank marker, left knee angle and left knee angular velocity for the 3 axes: [0] X, [1] Y and [2] Z.}
...
\begin{figure}[ht]
    ...
    \par\end{centering}
    \caption{Frames 500 to 1000 of the three axes: [0] X, [1] Y and [2] Z of the left knee angle, annotated with touchdown (\(L\_TD\)) and toe-off (\(L\_TO\)) events.}
...
Tabular:
1832 recordings of a sessions of 1402 distinct runners with 209 of them having more than 1 session.

Data Quality and standarisation:
The values of nominal categorical variables with less than 100 categories have been mapped to a code that is easier to work with(no spaces, no special caracters and restricted length), and we have consoldiated categories that refer to the same code but were written slightly differently (i.e. meniscal tear-lateral and meniscal tear lateral).
...
Quantitative variables have been left unchanged, standarisation and normalization has been left for a later stage.

All values that do not fall into a defined category or an accepted value for the datatype are considered as missing value and are converted to None. This is saved as "" when saving the dataset to CSV format.

Things to highly of the distributions of the columns:
- Male / female is balanced (find \%) and we have a "unknown" value.
- DominantLeg distibution skewed to Right leg as expected. We have few ambidextrous.
- Report distribution of \texttt{SpecInjury} and \texttt{InjDefn}
- Recreational and Competitive distribution skewed towards recreational
- Little longitudinal data. Only 30 sessions >= 1 month apart: 2.1 \%

TODO: Do we need a visualiztion to support?

Edge cases:
We comparing the sessions of the same subject we have found samples where the \texttt{Level} changes from competitive to recreational or the other way around, also we have found examples where the \texttt{DominantLeg} changes over time, and cases where both \texttt{SpecInjury} and \texttt{InjDefn} change. Since there is no indication in the source paper that makes us think that this is not possible, we will continue with assuming that this scenario is possible.

Edge cases that indicate that there is a data quality problem and that we need to act:
...
We override the age of the former to 41 and we create a new sinthetic subject ID (300375) and assign the second session to the new subject.

In some samples we have detected that SpecInjury is empty, but SpecInjury2 has a value. For those cases, we assume that it is an input error and move the value from SpecInjury to SpecInjury2.

% There are images in 04_eda..ipynb is needed
Outliers -> Replace with NaN
Find outliers drawing a boxplot and visually identifying unrealistic values.
...
Lastly, we introduce a surrogate key "id" that can be used accross datasets to identify each sample. One sample is a session. The surrogate key is the concatenation of subject id with the filename in uppercase i.e. "100001_20110531T161051". This will be needed when working with both timeseries and metadata.

The dataset, which only had metadata variables is enriched with the descriptive variables extracted from the source json files. We join both datasets by "id".

Missing data:
<table> Missing data, table with columns with most missing values after cleaning
As expected \textttt{SpecInjury2}, \textttt{InjJoint2} and \textttt{InjSide2} has the most missing values, it is only pupulated when a subject has two injuries reported. If it only has 1, then it will be empty. There are data quality issues with those columns, because they missing values \% do not match.
