...
In this chapter we present the materials provided in the source paper (\cite{Ferber2024}), we describe the source datsets, the preprocessing and exploration process, the implementation, training and evaluation of the models and the explainability analysis of the models.
...
<Draft> This project woukld have been set up and architected differently if our objective would've been the development and deployment of a injury detetion model alone. In such projects, model explainability is only a priority when debuging the model. On the other hand, one would put more attention to the easy of gathering the inputs, the model architecture and resource consumption. For our use case, model explainability is key to derive value from the work that has been done, as there is little value to society of having an AUC ROC of 0.7 on a black box Deep Learning model other than to prove that a good data science work has been done.

To build on top of this and extract the maximum value out of the project, we have made use of intrinsic methods and Explainable AI (XAI) techniques, as seen in \cite{FuentesJimnez2025}, to add transparency to model's decisions.
