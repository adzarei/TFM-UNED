...
\paragraph{Cross-validation within training}
Within the training set, a $k$-fold group-aware cross-validation procedure was employed to improve the reliability of model evaluation and to mitigate variance due to partitioning. Specifically, \texttt{k=5} folds were used, and splits were stratified by the injury status label while ensuring that all sessions from the same subject remained in the same fold.

\paragraph{Evaluation protocol}
Performance metrics were calculated on the validation folds during model selection and hyperparameter tuning. Once the best configuration was identified, the model was retrained on the entire training+validation data and finally evaluated once on the held-out test set. This protocol ensured that the reported test scores reflect generalisation to unseen subjects.
