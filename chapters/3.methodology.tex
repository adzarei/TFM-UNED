...
The outputs consist of time-series data for markers, joint angles, joint angular velocities, and detected gait cycle events with their corresponding frame indices. The outputs were further processed using the \texttt{pandas} and \texttt{kinetics-toolkit} Python packages.  

Figure \ref{fig:data-ext-marker-velo-angle-ts} illustrates an example from a single session, showing the raw marker data for one marker in the left shank cluster alongside the corresponding joint angle and angular velocity for the left knee. Figure \ref{fig:data-ext-l-knee-angle-events} shows frames 500-1000 of the left knee angle, annotated with the touchdown and toe-off events.


% % 3D Marker body
% \begin{figure}[ht]
%     \begin{centering}
%     \includegraphics[width=0.5\columnwidth]{images/ex_3d_marker.png}
%     \par\end{centering}
%     \caption{3D representation of a frame with all marker data. Connecting lines have been added between markers. The left foot has been painted in blue and the right foot in red. From top to botton: Pelvis, Left/Right Thigh, Left/Right Shank and Left/Right Foot.}
%     \label{fig:data-ext-marker-3d}
% \end{figure}

% % Marker, angles and velocities
% \begin{figure}[ht]
%     \begin{centering}
%     \includegraphics[width=0.5\columnwidth]{images/ex_marker_angle_vel_ts.png}
%     \par\end{centering}
%     \caption{1st left shank marker, left knee angle and left knee angular velocity for the 3 axis: [0] X, [1] Y and [2] Z.}
