...
\paragraph{\texttt{gait\_kinematics.m}} Matlab script provided alongside the source paper, that calculates joint angles and joint angle velocities from \texttt{Dynamic Marker Data}, \texttt{Join Marker Neutral} and \texttt{Cluster Marker Neutral} data. It constructs anatomical segment coordinate systems from the neutral marker data, tracks segment motion during using the dynamic marker data and computes XYZ Cardan joint angles and angular velocities. It returns:
\begin{itemize}
...
\paragraph{\texttt{gait\_steps.m}} Matlab script provided alongside the source paper, that calculates the descriptive biomechanical variable set from \texttt{Dynamic Marker Data}, \texttt{Cluster Marker Neutral} data, \texttt{Angles} and \texttt{Velocities}. It returns:
...
\paragraph{\texttt{processing\_code\_example\_with\_outputs.m}} Matlab script implemented for this project that processes the metadata file and marker data json files to calculate the joint angles and joint angular velocities using \texttt{gait\_kinematics.m} and \texttt{gait\_steps.m}. Additionally it converts all data into tabular format for the exploratory data analysis (EDA) phase of the project. Sessions that do not have running data or that the preprocessing in matlab failed have been discarded.
