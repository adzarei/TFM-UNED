...
In this project, we have implemented an interative Curve Registration with DTW. We used the \texttt{dtw} python library \citet{Giorgino2009} for the estimation of the best alignment between two curves. The pseudo code for the algorithm can be found \ref{alg:curve-registration-dtw}:

\begin{algorithm}
\caption{Iterative Curve Registration with DTW\label{alg:curve-registration-dtw}}
\begin{algorithmic}[1]
...
\For{$\text{iter} \gets 0$ to $\text{max\_iter} - 1$}
    ...
    \ForAll{curve $\in$ curves}
        \State alignment $\gets \mathrm{DTW}(\text{curve}, \text{template}, \text{step\_pattern})$
        \State $(\text{path\_x}, \text{path\_y}) \gets (\text{alignment.index1}, \text{alignment.index2})$
        \State aligned $\gets \mathbf{0}_{T}$; counts $\gets \mathbf{0}_{T}$
